...
class proveedor extends BD {
    ...
    public function getallProviderByPage($page = 1, $where = '') {
        if (empty($page)) {
            $page = 1;
        }
        
        $campos = "*";
        return $this->consultar_by_page($this->_sql("*,p.estado AS estadoProveedor, t.descripcion AS desTipod", "", "", "idproveedor ASC"), $page);
    }
