var _guardarMods = function(tipo, frm, mod){
	if($(frm).validationEngine('validate') ==true){				
		_guardar("auditoria_financiera/ajax/save.php?type="+tipo, $(frm).serialize(), function(html_response){
...
var _loadFormulariosEdit = function(caso, tipo, frm, mod, datosPost){
	_ajax("auditoria_financiera/ajax/form_edit.php?case="+caso, datosPost, function(html_response){
		botones = [{
			text : "Guardar",
			click : function(){ 
...
var _anularReg =function(tipo,datosPost, mod){
	if(confirm('¿Esta seguro de desactivar este registro?')){
 		_guardar("auditoria_financiera/ajax/save.php?type="+tipo, datosPost, function(html_response){
			switch(html_response){
				case '1':
...
var _addAuditoria = function(idFactura){
	_loadContenido('auditoria_financiera/ajax/form_add.php?idfactura='+idFactura);
}
var _editAuditoria = function(idauditoria, idFactura){
	_loadContenido('auditoria_financiera/ajax/form_edit.php?idauditoria='+idauditoria+'&idfactura='+idFactura);
}
var _verAuditorias = function( idFactura){
	_loadContenido('auditoria_financiera/index_auditoria.php?idfactura='+idFactura);
}
var _anularAuditoria = function (id){
	_anularReg("nullAuditoria","id="+id, "Auditoría");
}
$(function(){
     _loadContenido('auditoria_financiera/index_factura.php');
});
